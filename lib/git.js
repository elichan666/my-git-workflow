...
class Git {
  ...
  async isInProgress() {
    try {
      ...
      // 检查各种进行中的操作
      const hasRebaseApply = fs.existsSync(path.join(gitDir, 'rebase-apply'));
      const hasRebaseMerge = fs.existsSync(path.join(gitDir, 'rebase-merge'));
      const hasMergeHead = fs.existsSync(path.join(gitDir, 'MERGE_HEAD'));
      const hasCherryPickHead = fs.existsSync(path.join(gitDir, 'CHERRY_PICK_HEAD'));
      const hasBisectLog = fs.existsSync(path.join(gitDir, 'BISECT_LOG'));
      
      // 如果有 rebase、cherry-pick 或 bisect，直接返回 true（这些需要用户手动处理）
      if (hasRebaseApply || hasRebaseMerge || hasCherryPickHead || hasBisectLog) {
        return {
          inProgress: true,
          type: hasRebaseApply || hasRebaseMerge ? 'rebase' : 
                hasCherryPickHead ? 'cherry-pick' : 'bisect',
          canContinue: false
        };
      }
      
      // 如果是 merge，需要进一步检查是否有未解决的冲突
      if (hasMergeHead) {
        try {
          const status = await this.getStatus();
          // 检查是否有冲突文件（未解决的冲突）
          const hasUnresolvedConflicts = Array.isArray(status.files) && 
            status.files.some(f => f && f.conflict && f.working_dir !== ' ' && f.working_dir !== '??');
          
          // 如果没有未解决的冲突，说明冲突已解决，可以继续完成合并
          if (!hasUnresolvedConflicts) {
            return {
              inProgress: true,
              type: 'merge',
              canContinue: true,
              message: '检测到未完成的合并，但冲突已解决。可以执行 git commit 完成合并，或使用 git merge --continue'
            };
          } else {
            return {
              inProgress: true,
   ...
   * 检查本地分支是否有未推送的提交
   */
  async hasLocalCommits(branchName) {
    try {
      // 先检查远程分支是否存在
      const hasRemote = await this.hasRemoteBranch(branchName);
      if (!hasRemote) {
        return false; // 没有远程分支，无法比较
      }
      ...
      // 如果本地和远程提交相同，说明没有未推送的提交
      if (localCommit === remoteCommit) {
        return false;
      }
      
      // 检查本地是否有远程没有的提交（本地领先于远程）
      try {
        const result = await this.git.raw(['rev-list', `${remoteCommit}..${localCommit}`]);
        return result.trim().length > 0;
      } catch {
        // 如果 rev-list 失败，可能是分支不存在，返回 false
        return false;
      }
    } catch (error) {
      // 如果获取失败，保守处理，返回 false
      return false;
    }
  }
