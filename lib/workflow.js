...
class Workflow {
  ...
  async toTest() {
    try {
      ...
      if (!hasRemote) {
        ...
        if (hasUpdates) {
          ...
          if (this.config.skipInteractive) {
            pullStrategy = this.config.pullStrategy;
            this.printInfo(`使用配置的拉取策略: ${pullStrategy}`);
          } else {
            pullStrategy = await this.prompter.selectPullStrategy(this.config.pullStrategy);
          }
      ...
      // 拉取 test 最新代码
      let testPullStrategy;
      if (this.config.skipInteractive) {
        testPullStrategy = this.config.pullStrategy;
        this.printInfo(`使用配置的拉取策略: ${testPullStrategy}`);
      } else {
        testPullStrategy = await this.prompter.selectPullStrategy(this.config.pullStrategy);
      }
      ...
      if (this.config.skipInteractive) {
        ...
        this.printInfo(`使用配置的合并策略: ${mergeStrategy}`);
      } else {
        mergeStrategy = await this.prompter.selectMergeStrategy(this.config.mergeStrategy);
      }
